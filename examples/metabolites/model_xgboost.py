...
if __name__ == '__main__':
    ...
    del metab_with_formula["formula"]

    tags = pd.read_csv("tag.csv")
    tags = np.array([int(i) for i in tags["IBD_Status"]]).astype(np.int64)  # binary

    # Divide input into train and test data
    train_data, test_data, train_labels, test_labels = train_test_split(metab_with_formula.T, tags, test_size=0.2,
                                                                        random_state=42)
    ...
    for train_indices, val_indices in k_fold.split(train_data, train_labels):
        ...
        val_preds = clf.predict(val_fold_data.values)
        val_accuracy = accuracy_score(val_fold_labels, val_preds)
    ...
    with open("results_xgboost.pkl", "wb") as f:
